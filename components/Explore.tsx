...
const getCategoryIcon = (category: string, size: number = 16, active: boolean = true) => {
  if (!category) return <Sparkles size={size} className="text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.8)]" />;
...
const createNeonIcon = (category: string) => {
  ...
  const lowerCat = category.toLowerCase();
  let color = '#ffffff'; 
  let iconPath = '<circle cx="12" cy="12" r="3"></circle>'; 
  ...
  } else if (lowerCat.includes('nature') || lowerCat.includes('outdoor')) {
    color = '#34d399';
  ...
  return L.divIcon({
    ...
    html: `
      <div style="position: relative; width: 42px; height: 42px; filter: drop-shadow(0 0 12px ${color}99);">
        <div style="width: 100%; height: 100%; background: ${color}; border: 2.5px solid white; border-radius: 50% 50% 50% 0; transform: rotate(-45deg);"></div>
        <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; z-index: 2; padding-bottom: 2px;">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">${iconPath}</svg>
        </div>
      </div>
    `,
    iconSize: [42, 42],
        ...
        <div className="w-full aspect-[16/10] relative bg-slate-100 overflow-hidden shrink-0">
          ...
          <div className="absolute bottom-6 left-8 right-8 text-white">
            <div className="flex items-center gap-1.5 mb-1 opacity-90">
              <div className="flex text-amber-400">{[...Array(5)].map((_, i) => <Star key={i} size={10} fill={i < Math.floor(activity.rating) ? "currentColor" : "none"} stroke="currentColor" />)}</div>
              <span className="text-[9px] font-black tracking-widest uppercase">{activity.rating} Score</span>
            </div>
            <h3 className="text-3xl font-black italic tracking-tighter leading-none uppercase">{activity.name}</h3>
          </div>
        </div>
        <div className="flex-1 p-8 md:p-10 overflow-y-auto bg-white custom-scrollbar">
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <p className="text-slate-600 leading-relaxed font-medium text-base">{sanitizeDescription(activity.description)}</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
              <a href={mapsUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-3 bg-indigo-600 text-white py-5 rounded-[1.8rem] font-black uppercase tracking-widest text-[10px] shadow-xl hover:bg-indigo-700 transition-all"><Navigation size={18} /> Get Directions</a>
              <button onClick={() => onWishlist(activity.id)} className={`flex items-center justify-center gap-3 py-5 rounded-[1.8rem] font-black uppercase tracking-widest text-[10px] transition-all border-2 ${isWishlisted ? 'bg-rose-50 border-rose-100 text-rose-500' : 'bg-white border-slate-100 text-slate-400 hover:border-rose-200 hover:text-rose-500'}`}>
...
const MapView: React.FC<{ 
  ...
  const activitiesWithCoords = useMemo(() => activities.map(a => ({ ...a, coords: parseLocation(a.location) })).filter(a => a.coords) as (Activity & { coords: { lat: number; lng: number } })[], [activities]);
    ...
    <div className="h-[65vh] w-full relative rounded-[2.5rem] overflow-hidden border border-slate-100 shadow-xl">
      <MapContainer center={munichCenter} zoom={13} scrollWheelZoom={true}>
        ...
        {activitiesWithCoords.map(activity => (
          <Marker key={activity.id} position={[activity.coords.lat, activity.coords.lng]} icon={createNeonIcon(activity.category)}>
            <Popup className="custom-popup">
              <div className="w-48 p-1">
                ...
                <div className="flex gap-1">
                  <button onClick={() => onSelect(activity)} className="flex-1 py-2 bg-slate-900 text-white text-[9px] font-black uppercase tracking-widest rounded-lg">Details</button>
                  <button onClick={() => onWishlist(activity.id)} className="p-2 bg-indigo-600 text-white rounded-lg"><Heart size={10} fill="currentColor" /></button>
                </div>
              </div>
            </Popup>
          </Marker>
...
const Explore: React.FC<ExploreProps> = ({ onWishlistUpdate }) => {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const currentUser = db.getUser();
        if (currentUser) {
          const rankedIds = await getPersonalizedRanking(currentUser, allActivities);
          setRecommendedIds(rankedIds);
        }
      } catch (err) { console.error(err); } finally { setLoading(false); }
    }
    loadData();
    const unsub = db.subscribeToGroups(setGroups);
    return () => unsub();
  }, []);

  const handleWishlist = async (id: string) => { await db.addToWishlist(id); onWishlistUpdate(); };
  ...
  const handleShareToGroup = async (activityId: string, groupId: string | null) => {
    ...
    await db.addMessage({
      text: isGlobal ? `Found this on Minglr!` : `Squad trip to ${activity.name}?`,
      activityId: activity.id,
      groupId: groupId || undefined,
      poll: isGlobal ? null : { question: `In for ${activity.name}?`, options: [{ text: "YES", votes: [] }, { text: "NO", votes: [] }] }
    });
    alert(`Shared!`);
  };

  const filteredActivities = selectedCategory 
    ? activities.filter(a => (a.category?.toLowerCase().includes(selectedCategory.toLowerCase()))) 
    : activities;
  ...
  return (
    <div className="p-6 pb-24 space-y-6 animate-in fade-in duration-700">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 space-y-4"><div className="w-12 h-12 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin"></div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Ranking best spots...</p></div>
      ) : (
        <div className="space-y-10">
          {viewMode === 'map' ? (
            <MapView activities={filteredActivities} onWishlist={handleWishlist} onSelect={setSelectedActivity} />
          ) : (
            <>
              ...
              <section className="space-y-6">
                <h3 className="font-black text-slate-400 uppercase tracking-widest text-[10px]">{selectedCategory ? `${selectedCategory} in Munich` : 'All Activities'}</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">{others.map(a => <ActivityCard key={a.id} activity={a} groups={groups} onWishlist={() => handleWishlist(a.id)} onShare={(gid) => handleShareToGroup(a.id, gid)} onClick={() => setSelectedActivity(a)} />)}</div>
              </section>
            </>
          )}
        </div>
      )}
      {selectedActivity && <ActivityDetailModal activity={selectedActivity} onClose={() => setSelectedActivity(null)} onWishlist={handleWishlist} />}
    </div>
  );
};

const ActivityCard: React.FC<{ activity: Activity; groups: ChatGroup[]; onWishlist: () => void; onShare: (gid: string | null) => void; onClick: () => void; }> = ({ activity, groups, onWishlist, onShare, onClick }) => {
  const { userLocation } = useLocation();
  const currentUser = db.getUser();
  const isWishlisted = currentUser?.wishlist?.includes(activity.id) || false;
  const [showGroupDropdown, setShowGroupDropdown] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => { if (dropdownRef.current && !dropdownRef.current.contains(e.target as Node)) setShowGroupDropdown(false); };
  ...
  const renderLocationText = (location: any) => {
    ...
    if (userLocation && coords) return `${formatDistance(calculateDistance(userLocation.lat, userLocation.lng, coords.lat, coords.lng))} away`;
    return typeof location === 'string' ? location : 'Munich, Germany';
  };

  const category = activity.category || (activity as any).type;

  return (
    <div onClick={onClick} className="group bg-white rounded-[2.5rem] overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 flex flex-col h-full cursor-pointer relative">
      <div className="relative aspect-video overflow-hidden bg-slate-100">
        ...
        <div className="absolute bottom-4 left-5 right-5 text-white">
          <div className="flex items-center gap-1.5 mb-0.5"><div className="flex text-amber-400">{[...Array(5)].map((_, i) => <Star key={i} size={8} fill={i < Math.floor(activity.rating) ? "currentColor" : "none"} stroke="currentColor" />)}</div><span className="text-[8px] font-black uppercase opacity-80">{activity.rating} Score</span></div>
          <h4 className="text-lg font-black italic tracking-tight drop-shadow-md leading-tight uppercase">{activity.name}</h4>
        </div>
      </div>
