...
const Login: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-indigo-50 via-white to-rose-50/30">
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          {/* Logo Section */}
          <div className="text-center mb-10">
            <div className="w-24 h-24 rounded-[2.5rem] overflow-hidden shadow-2xl shadow-indigo-100 mx-auto mb-6 bg-white flex items-center justify-center border border-slate-50">
              <img 
                src="./Minglr.png" 
                alt="Minglr" 
          ...
          {/* Auth Card */}
          <div className="bg-white rounded-[3rem] shadow-[0_30px_60px_rgba(79,70,229,0.08)] overflow-hidden border border-slate-100">
            ...
            <div className="p-8 space-y-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                {isSignUp && (
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2 ml-1">
                      <User size={10} className="text-indigo-400" /> Display Name
                    </label>
                    <input
                      type="text"
                      value={displayName}
                ...
                <div className="space-y-1.5">
                  <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2 ml-1">
                    <Mail size={10} className="text-indigo-400" /> Email Address
                  </label>
                  <input
                    type="email"
                    value={email}
                ...
                <div className="space-y-1.5">
                  <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2 ml-1">
                    <Lock size={10} className="text-indigo-400" /> Password
                  </label>
                  <input
                    type="password"
                    value={password}
                ...
                {error && (
                  <div className="bg-rose-50 border border-rose-100 text-rose-600 px-5 py-3 rounded-xl text-xs font-bold">
                    {error}
                  </div>
                )}
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-5 bg-indigo-600 text-white rounded-[2rem] font-black uppercase tracking-[0.2em] shadow-xl shadow-indigo-100 hover:bg-indigo-700 disabled:opacity-50 transition-all active:scale-[0.98] flex items-center justify-center gap-3"
                >
                  {loading ? (
      ...
      <footer className="shrink-0 py-4 border-t bg-white/80 backdrop-blur-sm text-center">
        <p className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.15em] text-slate-900">
          Created by Sudarshan Dahal, copyright 2025
        </p>
      </footer>
    </div>
  );
